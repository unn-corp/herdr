...
- `default_cwd` sidebar token, showing a space's default working directory under its name. Included in the default space rows; the row collapses for spaces without one.
- Added `ui.context_usage` to show per-pane AI agent context-window usage in the system monitor strip, to the left of the git branch spans, for example `ctx claude 63% ▰▰▰▰▰▰▱▱ 2h14m`. Usage is reported over the socket API with `pane.report_usage` and surfaced on `PaneInfo.usage`, and is collected by the bundled `plugins/context-usage` crate. Reset countdowns appear only when a provider supplies a machine-readable reset time, never synthesized. Config keys: `enabled`, `bar_width`, `show_reset`, `show_model`. Collectors: Claude Code (statusLine `rate_limits`, push), Codex (rollout `token_count` / `model_context_window`), and OpenCode (SQLite session store + a model context-window registry), the latter two pulled by `herdr-context-usage poll` which maps each pane to its session by working directory. `[ui.context_usage.native]` sets a per-agent preference (`prefer-herdr` / `prefer-native` / `both` / `hidden`); Hermes defaults to `prefer-native` so Herdr does not duplicate its own context bar.
