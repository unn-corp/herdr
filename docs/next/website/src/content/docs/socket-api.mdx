...
| Workspace | `workspace.create`, `workspace.list`, `workspace.get`, `workspace.focus`, `workspace.rename`, `workspace.move`, `workspace.set_cwd`, `workspace.report_metadata`, `workspace.close` |
...
| Pane | `pane.split`, `pane.swap`, `pane.move`, `pane.zoom`, `pane.layout`, `pane.process_info`, `pane.neighbor`, `pane.edges`, `pane.focus_direction`, `pane.resize`, `pane.list`, `pane.current`, `pane.get`, `pane.rename`, `pane.send_text`, `pane.send_keys`, `pane.send_input`, `pane.read`, `pane.graphics.info`, `pane.graphics.set`, `pane.graphics.clear`, `pane.graphics.stream`, `pane.report_agent`, `pane.report_agent_session`, `pane.report_metadata`, `pane.report_usage`, `pane.clear_agent_authority`, `pane.release_agent`, `pane.close`, `pane.wait_for_output` |
| Popup | `popup.close` |
...
Use `pane.report_usage` to report a pane's AI agent context-window usage. Herdr stores it on the pane, surfaces it as the `usage` field of `PaneInfo`, and, when `[ui.context_usage]` is enabled, renders it in the monitor strip. This powers the bundled `context-usage` plugin.

```json
{
  "id": "req_3",
  "method": "pane.report_usage",
  "params": {
    "pane_id": "w1:p1",
    "source": "herdr-context-usage:claude-statusline",
    "agent": "claude",
    "model": "claude-sonnet-4-20250514",
    "used_pct": 63,
    "context_window_tokens": 200000,
    "used_tokens": 126000,
    "reset_at_unix": 1783468800,
...
Usage reports are display-only. Every field except `pane_id` and `source` is optional; a reader treats a missing field as unknown, not zero. `used_pct` must be between `0` and `100`. `confidence` must be one of `official`, `estimated`, `heuristic`, or `unavailable`. Set `reset_at_unix` only from a machine-readable provider signal; it is never synthesized. Pass `clear: true` with the same `source` to remove a pane's usage. `source` follows the same rules as `pane.report_metadata` (80 characters or fewer, ASCII letters, digits, colon, dot, underscore, and hyphen), and `ttl_ms` and `seq` behave the same way; when the TTL expires the usage stops rendering.
