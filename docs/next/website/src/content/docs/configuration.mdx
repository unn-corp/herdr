...
Set `system_monitor = true` to show a one-row CPU / RAM / GPU usage strip at the top of each space. CPU comes from `/proc/stat` and memory from `/proc/meminfo`; GPU utilization is read from `nvidia-smi` first, then an AMD `gpu_busy_percent` sysfs node, and the GPU segment is hidden when neither is available. `system_monitor_interval_ms` sets the refresh cadence (default 2000). The GPU read runs off the render loop so it never stalls the UI. System metrics are read on Linux; other platforms show CPU and RAM as unavailable.
