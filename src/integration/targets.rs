...
/// Install Grok (xAI CLI) session reporting so Herdr can resume panes with
/// `grok --resume <session_id>` after a server restart.
pub(crate) fn install_grok() -> io::Result<GrokInstallPaths> {
    let dir = grok_dir()?;
    // Create ~/.grok (and hooks/) even if the CLI hasn't been launched yet —
    // presence of `grok` on PATH is enough to install.
    let hooks_dir = dir.join("hooks");
    fs::create_dir_all(&hooks_dir)?;
    ...
    // Fully managed file: Grok discovers ~/.grok/hooks/*.json and merges them.
    // Owning the whole file keeps install/uninstall simple and idempotent.
    let hooks_json = json!({
        "hooks": {
            "SessionStart": [
                {
