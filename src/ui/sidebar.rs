...
"##,
    ...
    fn collapsed_sidebar_numbers_priority_agents_by_list_position() {
        ...
        // This fork paints agent state with fixed status colors (ui/status.rs STATE_*) rather than
        // the theme palette, so the blocked glyph is STATE_BLOCKED, not palette.red. Upstream's
        // version of this assertion encodes upstream's palette-based styling.
        assert_eq!(
            buffer[(detail_area.x + 2, detail_area.y)].style().fg,
            Some(crate::ui::status::STATE_BLOCKED)
        );
    }
